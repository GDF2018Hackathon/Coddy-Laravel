...
class SnifController extends Controller
{
    ...
    //     $this->exeCommand('git clone '. $repo_clone .' '.$scan_dir);
    //     $this->exeCommand('cd '.$scan_dir.' && git checkout '.$branch);
    //   }
    //   else{
    //     $this->exeCommand('cd '.$scan_dir.' && git pull');
    //     $this->exeCommand('cd '.$scan_dir.' && git checkout '.$branch);
    //   }
    //
    //   $ignore = [
    ...
    //   $scan_dir = $id . $path;
    //
    //   $result = $this->exeCommand('phpcs --ignore='. implode(',', $ignore) .' --standard=PSR2 --report=json '.$scan_dir);
    //
    //   $report = new Report;
